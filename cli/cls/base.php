...
class LEM_BaseLoader {
    ...
    static function _loadClass($className, $dir = '', $isGetInstance = true, $classPrefix = 'LEM_') {
            ...
            $fileDir = S_ROOT ."cli/". $dir . strtolower($className) . '.class.php';
            if (!$isGetInstance) return (require_once S::escapePath($fileDir, false)); //未实例化的直接返回
            $class = $classPrefix . $className;
            if (!class_exists($class)) { 
                    if (file_exists($fileDir)) require_once S::escapePath($fileDir, false);
