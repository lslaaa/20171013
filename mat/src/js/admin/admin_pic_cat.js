...
var admin_pic_cat = {
	...
	_init:function(){
		...
		$('#J_add_level_one').click(function(){
								...
								_data.pic  = '';
								_data['sort'] = 200;
								var _html = admin_pic_cat.make_html(_data);
		...
		$('.J_show_hide_menu_mod').click(function(){
								...
								_ajax_jsonp('/admin.php?mod=pic&mod_dir=pic&extra=get_one_cat&cid='+int_page_id,admin_pic_cat._mod);
								c._this = _this;
							});
		$('.J_show_hide_menu_del').click(function(){
								$('.J_submit_from').html('');
								var _this = $(this);
								confirm_cancle(admin_pic_cat._del,arr_language['warn'],arr_language['del_2']);
								c._this = _this;
							});
	},
	...
	_del:function(){
		var _this = admin_pic_cat._this;
		var obj_data = _this.parent().parent();
		var int_cid = obj_data.attr('data-cid');
		//_ajax_jsonp('/admin.php?mod=item_cat&extra=del&mod_dir=mall&mod_dir=mall&cid='+int_cid,admin_pic_cat._del_success);
	},
	...
	verify:function(){
		...
		*/
		...
		if(admin_pic_cat.bool_sending){
			return false;	
		}
		_ajax_frame(get_obj('ajaxframeid'),admin_pic_cat.send_success,'send_success');
		$('#J_send_form').submit();
		admin_pic_cat.bool_sending = true;
	},
	send_success:function(_data){
		_remove_ajax_frame('send_success');
		//console.log(_data);
		if(_data['status']!=200){
			admin_pic_cat.bool_sending = false;
