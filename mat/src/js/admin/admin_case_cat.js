...
var admin_case_cat = {
	...
	_init:function(){
		$('#J_add_level_one').click(function(){
								...
								_data.url  = '';
								_data['sort'] = 200;
								var _html = admin_case_cat.make_html(_data);
								_this.parent().next('.J_submit_from').html(_html);
								admin_case_cat.listen_send();
							});
		...
		$('.J_show_hide_menu_mod').click(function(){
								...
								_ajax_jsonp('/admin.php?mod=case&mod_dir=case&extra=get_one_cat&cid='+int_page_id,admin_case_cat._mod);
								admin_case_cat._this = _this;
							});
		$('.J_show_hide_menu_del').click(function(){
								$('.J_submit_from').html('');
								var _this = $(this);
								confirm_cancle(admin_case_cat._del,arr_language['warn'],arr_language['del_2']);
								admin_case_cat._this = _this;
							});
	},
	...
	_mod:function(_data){
		//console.log(_data);
		var _this = admin_case_cat._this;
	...
	_del:function(){
		...
		//_ajax_jsonp('/admin.php?mod=item_cat&extra=del&mod_dir=mall&mod_dir=mall&cid='+int_cid,admin_case_cat._del_success);
	},
	...
	verify:function(){
		...
		*/
		...
		if(admin_case_cat.bool_sending){
			return false;	
		}
		_ajax_frame(get_obj('ajaxframeid'),admin_case_cat.send_success,'send_success');
		$('#J_send_form').submit();
		admin_case_cat.bool_sending = true;
	},
	send_success:function(_data){
		_remove_ajax_frame('send_success');
		//console.log(_data);
		if(_data['status']!=200){
			admin_case_cat.bool_sending = false;
