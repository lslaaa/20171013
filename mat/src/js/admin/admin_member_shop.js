var admin_member_shop = {
        _init:function(){
                var c = admin_member_shop
                c._listen()
                c._listen_edit_pwd()
        },
        _listen:function(){
                $('#J_send_form').submit(function(){
                                                        if (!admin_member_shop._verify()) {
                                                                return false;
                                                        }
        ...
        _verify:function(){
                ...
                //检查数据是否符合提交条件
                if(!$('#J_gid').val()){
                    make_alert_html('系统提示','请选择用户组');
                                return false;
                }
                if(!$('#J_username').val()){
                    make_alert_html('系统提示','请填写用户名');
                                return false;
                }
                if(!$('#J_mobile').val()){
        ...
        _listen_edit_pwd:function(){
            $('#J_edit_pwd').click(function(){
                ...
                var _html = '<div id="J_open_div" class="pop_wrap" style="width:400px;">'+
                                                ...
                                                '</div>'+
                                                '<div class="pop_main">'+
                                                        '<form id="J_edit_form" action="/admin.php?mod=shop&extra=mod_detail" method="post" target="ajaxframeid" >'+
...
var admin_member_shop_del = {
        _init:function(){
                admin_member_shop_del._listen_del();
        },
        _listen_del:function(){
                var c = admin_member_shop_del;
        ...
        _del:function(){
                ...
                _ajax_jsonp('/admin.php?mod=shop&extra=del&sid='+int_sid,c._del_success);
        },
