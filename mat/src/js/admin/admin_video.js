var admin_video = {
	...
	_listen:function(){
		...
		$('#J_send_form').submit(function(){
							...
							var obj = $('#J_video_code');
							if($('.J_video_box').css('display')!='none'){
								if(!$('#J_video_url').val() && !obj.val()){
									make_alert_html(arr_language['warn'],arr_language['pics']['error_4']);
									return false;
								}
							}
							...
							if(!$('#J_video_url').val() && !obj.val()){
								make_alert_html(arr_language['warn'],arr_language['pics']['error_5']);
								return false;	
							}
	...
	_make_pic_html:function(){
		var _html = '<dl class="J_pic_box mt12">'+
						...
						'<dd style="position:relative;" id="J_img_add_dd">'+
							...
							'<span><a href="javascript:;" class="J_del_pic del_lsit nico" title="ɾ��"><!--ɾ��--></a></span>'+
	...
	_listen_cat:function(){
		...
		obj_cid.change(function(){
						...
						var int_pid = _this.val();
                                                var tx = _this.find("option:selected").attr('p_type');
                                                if(tx == 'video'){
                                                    $('.J_video_box').show();
                                                }
	...
	_del:function(){
		var c = admin_video;
		var _this = admin_video._this;
		var int_nid = _this.attr('data-id');
		_ajax_jsonp('/admin.php?mod=video&mod_dir=video&extra=del&nid='+int_nid,c._del_success);
	},
	...
	send_success:function(_data){
		...
		make_alert_html(arr_language['notice'],_data['info'],'/admin.php?mod=video&mod_dir=video');
	}
}

var admin_video_search = {
	_init:function(){
		admin_video_search._listen_cat();	
	},
	_listen_cat:function(){
		var obj_cid_1 = $('#J_cid');
