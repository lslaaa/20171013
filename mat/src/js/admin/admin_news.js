var admin_news = {
	...
	_listen:function(){
		...
		$('#J_send_form').submit(function(){
							...
							if(obj_cat.css('display')!='none'){
								var obj_cid = $('#J_cid');
								if(obj_cid.val()<=0){
									make_alert_html(arr_language['warn'],arr_language['news']['error_2']);
									return false;	
								}
							...
							if(!obj_title.val()){
								make_alert_html(arr_language['warn'],arr_language['news']['error_3']);
								return false;	
							}
	...
	_del:function(){
		var c = admin_news;
		var _this = admin_news._this;
		var int_nid = _this.attr('data-nid');
		_ajax_jsonp('/admin.php?mod=news&mod_dir=news&extra=del&nid='+int_nid,c._del_success);
	},
	...
	send_success:function(_data){
		...
		make_alert_html(arr_language['notice'],_data['info'],'/admin.php?mod=news&mod_dir=news');
	}
}

var admin_news_search = {
	_init:function(){
		admin_news_search._listen_cat();	
	},
	_listen_cat:function(){
		var obj_cid_1 = $('#J_cid');
