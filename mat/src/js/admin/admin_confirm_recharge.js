var admin_confirm_recharge = {
	_init:function(){
		var c = admin_confirm_recharge;
		c._listen_reply();
	},
	_listen_reply:function(){
		...
		$('.J_btn_reply').click(function(){
								...
								var _html = '<div id="J_open_div" class="pop_wrap" style="width:400px;">'+
												'<div class="pop_title">'+
													'<span class="f_l">确认充值</span>'+
													'<a id="J_close_tan" class="close f_r" href="javascript:void(0);">'+arr_language['close']+'</a>'+
												'</div>'+
												'<div class="pop_main">'+
													'<form id="J_send_form" action="/admin.php?mod=money&mod_dir=money&extra=confirm_recharge" method="post" target="ajaxframeid" >'+
														'<input class="J_callback" type="hidden" value="parent.send_success" name="callback">'+
														'<input type="hidden" value="200" name="status">'+
														...
														'<input type="submit" style="border-radius:3px;height:52px; line-height:52px; width:158px; border:0; font-weight:700; font-size:16px;" value="确定" class="blue_nav">'+
													'</form>'+
												'</div>'+
											'</div>';
