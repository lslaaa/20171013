...
var admin_mall_item_cat = {
	...
	_init:function(){
		$('#J_add_level_one').click(function(){
								...
								_data.url  = '';
								_data['sort'] = 200;
								var _html = admin_mall_item_cat.make_html(_data);
								_this.parent().next('.J_submit_from').html(_html);
								admin_mall_item_cat.listen_send();
							});
		...
		$('.J_show_hide_menu_mod').click(function(){
								...
								_ajax_jsonp('/admin.php?mod=item_cat&extra=get_one&mod_dir=mall&cid='+int_cid,admin_mall_item_cat._mod);
								admin_mall_item_cat._this = _this;
							});
		$('.J_show_hide_menu_del').click(function(){
								$('.J_submit_from').html('');
								var _this = $(this);
								confirm_cancle(admin_mall_item_cat._del,arr_language['warn'],arr_language['del_2']);
								admin_mall_item_cat._this = _this;
							});
	},
	...
	_mod:function(_data){
		//console.log(_data);
		var _this = admin_mall_item_cat._this;
	...
	_del:function(){
		...
		var obj_data = _this.parent().parent();
		var int_cid = obj_data.attr('data-cid');
		_ajax_jsonp('/admin.php?mod=item_cat&extra=del&mod_dir=mall&mod_dir=mall&cid='+int_cid,admin_mall_item_cat._del_success);
	},
	...
	verify:function(){
		var obj_name = $('#J_name');
		if(!obj_name.val()){
			make_alert_html(arr_language['warn'],arr_language['mall']['item_cat']['error_1']);
			return false;
		}
		if(typeof(json_languages)=='object'){
			for(var i=0,ci;ci=json_languages[i++];){
		...
		*/
		...
		if(admin_mall_item_cat.bool_sending){
			return false;	
		}
		_ajax_frame(get_obj('ajaxframeid'),admin_mall_item_cat.send_success,'send_success');
		$('#J_send_form').submit();
		admin_mall_item_cat.bool_sending = true;
	},
	send_success:function(_data){
		_remove_ajax_frame('send_success');
		//console.log(_data);
		if(_data['status']!=200){
			admin_mall_item_cat.bool_sending = false;
