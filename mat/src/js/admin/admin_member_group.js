var admin_member_group = {
	_this:'',
	_init:function(){
		admin_member_group._listen();	
	},
	_listen:function(){
		$('#J_send_form').submit(function(){
							...
							if(admin_member_group.bool_sending){
								return false;	
							}
							admin_member_group.bool_sending = true;
							_ajax_frame(get_obj('ajaxframeid'),admin_member_group.send_success,'send_success');			  
						});
	},
	_listen_del:function(){
		$('.J_btn_del').click(function(){
								var _this = $(this);
								confirm_cancle(admin_member_group._del,arr_language['warn'],arr_language['del_2']);
								admin_member_group._this = _this;
							});
	},
	_del:function(){
		var _this = admin_member_group._this;
		var int_gid = _this.attr('data-gid');
		_ajax_jsonp('/admin.php?mod=member_group&mod_dir=setting&extra=del&gid='+int_gid,admin_member_group._del_success);
	},
	...
	send_success:function(_data){
		...
		//console.log(_data);
		if(_data['status']!=200){
			admin_member_group.bool_sending = false;
			make_alert_html(arr_language['warn'],_data['info']);
			return false;	
		}
		make_alert_html(arr_language['notice'],_data['info'],'/admin.php?mod=member_group&mod_dir=setting');
	}
}

var admin_member_group_permission = {
	_init:function(){
		var c = admin_member_group_permission;
		c._listen();
		c._listen_select();
	},
