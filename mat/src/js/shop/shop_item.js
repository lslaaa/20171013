var shop_item = {
	...
	_listen:function(){
		...
		$('#J_send_form').submit(function(){
					...
					$("#J_form_do").val('add');	
					if (!shop_item._verify()) {
						return false;
					}
					if(shop_item.bool_sending){
						return false;	
					}
					shop_item.bool_sending = true;
					_ajax_frame(get_obj('ajaxframeid'),shop_item.send_success,'send_success');			  
				});
	...
	send_success:function(_data){
		...
		if(_data['status']!=200){
			shop_item.bool_sending = false;
			make_alert_html(arr_language['warn'],_data['info'],'/shop.php?mod=item');
			return false;	
		}
		make_alert_html(arr_language['notice'],_data['info'],'/shop.php?mod=item');
	},
	//批量下架
    	_listen_bat_inactive:function(){
	        $('#J_bat_inactive').click(function(){
	                ...
	                _str_ids = str_ids;
	                confirm_cancle(shop_item.bat_inactive,arr_language['warn'],arr_language['del_3']);
	                shop_item._this = _this;
         	});
    	},
        
    	bat_inactive:function(){
		_ajax_jsonp('/shop.php?mod=item&extra=bat_inactive&ids='+_str_ids,shop_item.bat_inactive_success);
	},
...
var shop_item_del = {
	...
	_del:function(){
		...
		_ajax_jsonp('/shop.php?mod=item&extra=del&item_id='+int_item_id,c._del_success);
	},
