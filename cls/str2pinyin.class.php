 ...
 *       中文字符串：返回全拼音字符         eg. 测试字符串 => ceshizifuchuan
 *       中英混合串: 返回全拼音字符和英文   eg. 我i北京大学中文a4156s系j => woibeijingdaxuezhongwena4156sxij
 *  $py = new LEM_str2pinyin();
 *  $result = $py->get_spelling('我i北京大学中文a4156系j');
 
 *  2、首字母获取
 *  注： 英文的字串：不变返回(包括数字)     eg. abc123 => abc123
 *       中文字符串：返回拼音首字符         eg. 测试字符串 => CSZFC
 *       中英混合串: 返回拼音首字符和英文   eg. 我is北京大学中文a4156ss系j => WIBJDXZWA4156SXJ
 *  $py = new LEM_str2pinyin();
 *  $result = $py->get_initials('我i北京大学中文a4156系j');
 */
class LEM_str2pinyin{
	...
	*/
	...
	//无法识别的字
	private $arr_special_word = array(
								'昵'
							);
	private $arr_special_word_replace = array(
											'你'
										);
	 ...
	 * @return string
	 */
	public function get_spelling($str, $utf8 = true,$str_split='')
	{		
		//参数分析
		if (!$str) {
			return false;
		}
		$str = str_replace($this->arr_special_word,$this->arr_special_word_replace,$str);
		//编码转换.
		...
		for ($i=0; $i<$num; $i++) {
			...
			$pinyin .= $this->num2str($temp).$str_split;
		}
				
		//输出的拼音编码转换
		$pinyin = str_replace(' ','',$pinyin);
		$pinyin = rtrim($pinyin,$str_split);
     ...
     * 获取中文字串的拼音首字符
     *
     * @param string $str		所要转的汉字
	 * @param boolean $utf8 	汉字编码是否为utf8
	 * @return string
     */
    public function get_initials($str, $utf8 = true)
    {
        if ( empty($str) ) return '';
		/*
		...
		*/
		...
		//输出的拼音编码转换.
		$strreturn = strtoupper(implode('',$result));
		$strreturn = str_replace(' ','',$strreturn);
		return ($utf8==true) ? $this->iconvStr('gbk', 'utf-8', $strreturn) : $strreturn;
    }
	/**
