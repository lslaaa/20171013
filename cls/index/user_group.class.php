...
class LEM_user_group extends mysqlDBA {
   
    function get_one($int_gid) {
        $str_sql = "SELECT * FROM " . $this->index('user_group') . " WHERE `gid`='$int_gid' AND `is_del`=0";
    ...
    function update($arr_data, $arr_data2) {
        $str_sql = "UPDATE " . $this->index('user_group') . " SET " . make_sql($arr_data2, 'update') . " WHERE " . make_sql($arr_data, 'where');
    ...
    function insert($arr_data) {
        $sql = "INSERT INTO " . $this->index('user_group') . make_sql($arr_data, 'insert');
        ...
        */
        function verify_rename($str_group_name,$bool_return_data=false){
                $sql = "SELECT * FROM " . $this->index('user_group') . " WHERE `group_name`='{$str_group_name}' AND `is_del`=0";
    ...
    function get_list($arr_data,$int_page=1,$int_page_size=10,$str_orderby='`sort` ASC,`gid` ASC',$bool_only_total=false) {
        ...
        $arr_return = array();
                $str_sql = "SELECT COUNT(`gid`) FROM " . $this->index('user_group')." WHERE ".$str_where;
                $arr_return['total'] = $this->db->get_value($str_sql);
                if($bool_only_total){
                        return $arr_return['total'];
                }
                $str_sql = 'SELECT * FROM ' . $this->index('user_group') .' WHERE '. $str_where .' ORDER BY '.$str_orderby.$this->page_start($int_page,$int_page_size);
