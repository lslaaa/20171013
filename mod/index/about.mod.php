...
class mod_about {
	...
	function extra_index(){
		...
		$arr_banner = $obj_ads->get_list(array('ads_cid'=>7,'is_del'=>0),1,1);
		$arr_banner && $arr_banner = current($arr_banner['list']);
		//var_export($arr_banner);
		$arr_page_cat = $obj_page->get_cat_list(array('pid'=>1,'is_del'=>0),1,100);
		$int_page_id  = $int_page_id ? $int_page_id : $arr_page_cat[0]['page_id'] ;
		...
		include template('template/index/about');
	}
}