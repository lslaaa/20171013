...
class mod_user_group {
	...
	function extra_index(){
		...
		$arr_list = $obj_user_group->get_list($arr_data,$int_page,$int_page_size);
		...
		include template('template/admin/shop/user_group/index');
	}
	...
	function extra_add(){
		...
		if(submit_check('formhash')){
			...
			if(empty($str_group_name)){
				callback(array('info'=>$arr_language['user_group']['error_1'],'status'=>304));
			}
			...
			if(empty($int_gid)){
				$bool = $obj_user_group->verify_rename($str_group_name);
				if($bool){
					callback(array('info'=>$arr_language['user_group']['rename'],'status'=>304));	
				}
				$obj_user_group->insert($arr_data);
				callback(array('info'=>$arr_language['user_group']['add_success'],'status'=>200));
			}
			$arr_group = $obj_user_group->verify_rename($str_group_name,true);
			if($arr_group && $arr_group['gid']!=$int_gid){
				callback(array('info'=>$arr_language['user_group']['rename'],'status'=>304));		
			}
			$obj_user_group->update(array('gid'=>$int_gid),$arr_data);
			callback(array('info'=>$arr_language['ok_2'],'status'=>200));
			
		}
		$arr_group = $obj_user_group->get_one($int_gid);
		//var_export($arr_member);
		include template('template/admin/shop/user_group/add');
	}
	...
	function extra_del(){
		...
		$obj_user_group = L::loadClass('user_group','index');
		$obj_user_group->update(array('gid'=>$int_gid),array('is_del'=>1));
		callback(array('info'=>$arr_language['ok_2'],'status'=>200));
	}
	...
	function extra_set_permission(){
		...
		if(submit_check('formhash')){
			...
			$obj_user_group->update(array('gid'=>$int_gid),array('permission'=>implode(',',$arr_mid)));
			callback(array('info'=>$arr_language['ok_2'],'status'=>200));
		}
		...
		// $obj_setting_menu = L::loadClass('admin_setting_menu','index');
		$arr_data = $_SGLOBAL['shop_menu'];
		$arr_menu = array();
		...
		//var_export($arr_menu);
		include template('template/admin/shop/user_group/permission');	
	}
