...
class mod_member_group {
	...
	function extra_index(){
		...
		include template('template/admin/setting/member_group/index');
	}
	...
	function extra_add(){
		...
		if(submit_check('formhash')){
			...
			if(empty($int_gid)){
				...
				callback(array('info'=>$arr_language['admin_member_group']['add_success'],'status'=>200));
			}
		...
		include template('template/admin/setting/member_group/add');
	}
	...
	function extra_set_permission(){
		...
		//var_export($arr_menu);
		include template('template/admin/setting/member_group/permission');	
	}
