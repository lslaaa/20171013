...
class Uploader
{
    ...
    private function upFile()
    {
		...
		if($_GET['max_size']){
			...
			save_image($file["tmp_name"],$_GET['max_size'],$this->filePath);
...
function save_image($pic,$str_size,$str_file_path){
	global $_SGLOBAL;
	...
	foreach($arr_size as $v){
		...
		if(empty($int_new_width) && $int_new_height>0 && $int_new_height>$int_height){
			$int_new_height = $int_height;
			$int_new_width  = intval($int_width*($int_new_height/$int_height));
		}
		elseif(empty($int_new_height)&& $int_new_width>0 && $int_new_width>$int_width){
			$int_new_width = $int_width;	
			$int_new_height = intval($int_height*($int_new_width/$int_width));
		}
		elseif(empty($int_new_width)&&$int_new_height){
			$int_new_width = intval($int_width*($int_new_height/$int_height));
		}
		...
		switch($ext){
			case 1: {
				imagegif($obj_new_image,$str_save_file_path, 100);
				break;
			}
			case 2: {
				imagejpeg($obj_new_image,$str_save_file_path, 100);
				break;
			}
			case 3: {
				imagepng($obj_new_image,$str_save_file_path, 9);
				break;
			}
		}
