<?php !defined('LEM') && exit('Forbidden');?><?php sub_template_check('template/admin/video/index|template/admin/header|template/admin/footer', '1508292815', 'template/admin/video/index');?><!DOCTYPE HTML>
<html>
<head>
...
<div class="admin_menu mt12">
        <span class="J_show_hide_menu">
            <a id="J_add_level_one" href="/admin.php?mod=video&mod_dir=video&extra=add" class="blue_nav"><?=$arr_language['add']?></a>
        </span>
...
<div class="info_wrap main_table mt10">
    <table>
    	<tr>
            <th class="" style="text-align:left; padding-left:10px;"><?=$arr_language['pic']?></th>
        ...
        <tr class="white">
            <td style="text-align:left; padding-left:10px;"><img style="width:80px;height:80px;" src="<?=$v['pic']?>"/></td>
            <?php if($v['cid_1']>0) { ?>
            <td>
            ...
            <td class="table_nav">
            	<?php if($arr_languages) { ?>
            	<a style="width:50px;" href="/admin.php?mod=video&mod_dir=video&extra=add&id=<?=$v['id']?>&copy=1" class="red_nav admin_nav"><?=$arr_language['copy']?></a>
                <?php } ?>
            	<a<?php if($arr_languages) { ?> style="width:50px;" <?php } ?> href="/admin.php?mod=video&mod_dir=video&extra=add&id=<?=$v['id']?>" class="blue_nav admin_nav"><?=$arr_language['mod']?></a>
...
admin_video.json_cat = <?=$json_cat?>;
admin_video._listen_del();
admin_video._listen_cat();
admin_video_search._init();
</script>
</div>
