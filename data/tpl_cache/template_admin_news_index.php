<?php !defined('LEM') && exit('Forbidden');?><?php sub_template_check('template/admin/news/index|template/admin/header|template/admin/footer', '1511886209', 'template/admin/news/index');?><!DOCTYPE HTML>
<html>
<head>
    ...
    <table>
        ...
        <tr class="white">
            ...
            <td class="table_nav">
                ...
                <?php } ?>
            	<a<?php if($arr_languages) { ?> style="width:50px;" <?php } ?> href="/admin.php?mod=news&mod_dir=news&extra=add&nid=<?=$v['nid']?>" class="blue_nav admin_nav"><?=$arr_language['mod']?></a>
                <a<?php if($arr_languages) { ?> style="width:50px;" <?php } ?> href="javascript:;" class="gray_nav admin_nav J_btn_del" onclick="return false;" data-nid="<?=$v['nid']?>"><?=$arr_language['del']?></a>
</td>
        </tr>
        <?php } } ?>
        <?php } else { ?>
