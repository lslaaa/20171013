<?php !defined('LEM') && exit('Forbidden');?><?php sub_template_check('template/admin/page/index|template/admin/header|template/admin/footer', '1511886205', 'template/admin/page/index');?><!DOCTYPE HTML>
<html>
<head>
...
<div class="admin_menu mt12">
        <span class="J_show_hide_menu">
            <a id="J_add_level_one" href="/admin.php?mod=page&extra=add" class="blue_nav"><?=$arr_language['add']?></a>
        </span>
...
<div class="info_wrap main_table mt10">
    <table>
    	<tr>
            <th class="" style="text-align:left; padding-left:10px;"><?=$arr_language['page']['page_name']?></th>
            <?php if($arr_languages) { ?>
        ...
        <tr class="white">
            <td style="text-align:left; padding-left:10px;"><?php if($arr_cat[$arr_cat[$v['page_id']]['pid']]['pid']>0) { ?><?=$arr_cat[$arr_cat[$arr_cat[$v['page_id']]['pid']]['pid']]['name']?>&nbsp;&nbsp;<?php } ?><?php if($arr_cat[$v['page_id']]['pid']>0) { ?><?=$arr_cat[$arr_cat[$v['page_id']]['pid']]['name']?>&nbsp;&nbsp;<?php } ?><?=$arr_cat[$v['page_id']]['name']?></td>
            <?php if($arr_languages) { ?>
            <td><?=$arr_languages_b[$v['language']]['name']?></td>
            <?php } ?>
            <td class="table_nav">
            	<?php if($arr_languages) { ?>
            	<a style="width:50px;" href="/admin.php?mod=page&extra=add&page_id=<?=$v['page_id']?>&language=<?=$v['language']?>&copy=1" class="red_nav admin_nav"><?=$arr_language['copy']?></a>
                <?php } ?>
            	<a<?php if($arr_languages) { ?> style="width:50px;" <?php } ?> href="/admin.php?mod=page&extra=add&language=<?=$v['language']?>&page_id=<?=$v['page_id']?>" class="blue_nav admin_nav"><?=$arr_language['mod']?></a>
                <a<?php if($arr_languages) { ?> style="width:50px;" <?php } ?> href="javascript:;" class="gray_nav admin_nav J_btn_del" onclick="return false;" data-page_id="<?=$v['page_id']?>" data-language="<?=$v['language']?>"><?=$arr_language['del']?></a>
</td>
        </tr>
        <?php } } ?>
        <?php } else { ?>
        <tr class="white">
            <td colspan="<?php if($arr_languages) { ?>3<?php } else { ?>2<?php } ?>"><div class="no-data"><?=$arr_language['nothing']?></div></td>
        </tr>
        <?php } ?>
    </table>
</div>
...
admin_page._listen_del();
admin_page_search._init();
</script>
</div>
